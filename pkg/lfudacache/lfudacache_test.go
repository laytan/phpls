...
func TestPut(t *testing.T) {
	...
	c.Put("test2", 101)
	e, ok = c.entries["test2"]
	is.True(ok)
	is.Equal(e.lastUsedAge, uint(2))
	is.Equal(e.timesUsed, uint(1))
	is.Equal(e.size, datasize.Bit)
	is.Equal(len(c.scores), 2)
	...
	c.Put("test3", 103)
	e, ok = c.entries["test3"]
	is.True(ok)
	is.Equal(e.lastUsedAge, uint(4))
	is.Equal(e.timesUsed, uint(1))
	is.Equal(e.size, datasize.Bit)
	is.Equal(len(c.scores), 3)
	...
	_, ok = c.entries["test1"]
	is.Equal(ok, false)
	_, ok = c.entries["test2"]
	is.True(ok)
	_, ok = c.entries["test3"]
	is.True(ok)
